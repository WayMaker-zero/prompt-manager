      ...
      >
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white flex items-center gap-4 tracking-tight">
            <div className="bg-gradient-to-br from-brand-400 to-brand-600 dark:from-brand-600 dark:to-brand-800 p-3 rounded-2xl shadow-lg shadow-brand-500/30">
          ...
          <motion.button 
            ...
            type="button" 
            onClick={onClose} 
            className="p-3 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-2xl transition-colors shadow-inner"
          >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="space-y-8 flex-1 overflow-y-auto pr-4 pb-4 custom-scrollbar">
            <div>
              ...
              <input
                autoFocus={!prompt}
                type="text"
                className="w-full bg-slate-100/50 dark:bg-slate-950/50 border-2 border-slate-200/50 dark:border-slate-800/50 rounded-2xl px-6 py-4 text-slate-900 dark:text-slate-100 text-lg focus:ring-4 focus:ring-brand-500/20 focus:border-brand-500 dark:focus:border-brand-500 outline-none transition-colors placeholder:text-slate-400 font-bold shadow-inner"
                placeholder={t.title}
                value={title}
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-end mb-3">
                ...
                <motion.button
                  ...
                  className="flex items-center space-x-2 text-sm font-bold bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 hover:bg-brand-100 dark:hover:bg-brand-900/50 px-4 py-2 rounded-xl transition-colors shadow-sm border border-brand-200 dark:border-brand-800"
              ...
              <textarea
                ref={textareaRef}
                className="w-full flex-1 min-h-[260px] bg-slate-100/50 dark:bg-slate-950/50 border-2 border-slate-200/50 dark:border-slate-800/50 rounded-3xl px-6 py-6 text-slate-900 dark:text-slate-100 focus:ring-4 focus:ring-brand-500/20 focus:border-brand-500 dark:focus:border-brand-500 outline-none transition-colors font-mono text-[16px] leading-loose resize-y custom-scrollbar placeholder:text-slate-400 shadow-inner"
          ...
          <div className="flex justify-end gap-4 pt-8 mt-auto">
            <motion.button
              ...
              type="button"
              onClick={onClose}
              className="px-8 py-4 text-base font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-2xl transition-colors shadow-inner border border-transparent"
            >
              {t.cancel}
            </motion.button>
            <motion.button
              ...
              className="px-8 py-4 text-base font-black bg-brand-600 text-white dark:bg-brand-600 rounded-2xl hover:bg-brand-700 dark:hover:bg-brand-500 transition-colors disabled:opacity-50 disabled:shadow-none shadow-xl shadow-brand-600/20"
            >
              {t.savePrompt}
            </motion.button>
          </div>
