...
export default function MainArea({ category, onUpdateCategory }: MainAreaProps) {
      ...
      <div className="flex-1 flex flex-col relative bg-[var(--bg-base)]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <motion.p 
            ...
            className="text-slate-500 dark:text-slate-400 text-xl font-bold tracking-tight"
          >
            {t.selectOrCreate}
          </motion.p>
        </div>
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--bg-base)] overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto px-10 pb-12 pt-2 custom-scrollbar relative">
        <AnimatePresence mode="wait">
          <motion.div
            key={category.id} // This key ensures the whole container animates nicely when category switches
          ...
          >
            {category.prompts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-slate-300 dark:border-slate-700/50 rounded-[3rem] bg-white/30 dark:bg-slate-900/30">
              ...
              <motion.div layout className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-8 auto-rows-max pb-12">
